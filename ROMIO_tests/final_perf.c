...
#define SIZE (1073741824)       /* read/write size per node in bytes */
...
int main(int argc, char **argv)
{
    ...
    double stim, size, read_tim, write_tim, new_read_tim, new_write_tim;
...
/* process 0 takes the file name as a command-line argument and 
   broadcasts it to other processes */
	...
	MPI_File_seek(fh, mynod*(SIZE/nprocs), MPI_SEEK_SET);

	MPI_Barrier(MPI_COMM_WORLD);
	stim = MPI_Wtime();
	MPI_File_write(fh, buf, SIZE/nprocs, MPI_BYTE, &status);
	...
	MPI_Barrier(MPI_COMM_WORLD);
	stim = MPI_Wtime();
	MPI_File_read(fh, buf, SIZE/nprocs, MPI_BYTE, &status);
    ...
    if (mynod == 0) {
	read_bw = (SIZE)/(min_read_tim*1024.0*1024.0);
	write_bw = (SIZE)/(min_write_tim*1024.0*1024.0);
