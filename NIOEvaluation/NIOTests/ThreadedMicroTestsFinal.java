...
class IntBufferWrite extends Thread {
    ...
    void setParameters(int pos, int[] ints, int nints, boolean debug, int warmup, int iterations) throws IOException {
        try {
            myNumInts = nints;
            iarr = ints;
            fc = FileChannel.open(Paths.get("ibuf_threaded_test.txt"),
    ...
    public void run() {
        try {
            ...
            for (int i = 0; i < WARM_UP; i++) {
                ...
                bbuf.clear();
                System.out.println(Thread.currentThread().getName() + " Iteration " + i + " Time " + end[i]);
            }
...
class MappedIntBufferWrite extends Thread {
    ...
    void setParameters(int pos, int[] ints, int nints, boolean debug, int warmup, int iterations) throws IOException {
        try {
            ...
            fc = FileChannel.open(Paths.get("mapped_ibuf_threaded_test.txt"),
                    StandardOpenOption.CREATE,
                    StandardOpenOption.WRITE,
                    StandardOpenOption.READ);
            position = pos;
...
class StreamsWrite extends Thread {
    ...
    void setParameters(int pos, int[] ints, int nints, boolean debug, int warmup, int iterations) {
        try {
            File file = new File("streams_threaded_test.txt");
            if (!file.exists()) {
...
class StreamsExtensionsWrite extends Thread {
    ...
    void setParameters(int pos, int[] ints, int nints, boolean debug, int warmup, int iterations) throws IOException {
        try {
            File file = new File("streams_extensions_threaded_test.txt");
            if (!file.exists()) {
                //System.out.println("File does not exist so creating a new file");
                try {
                    file.createNewFile();
